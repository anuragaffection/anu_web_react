...
function ImgRectangle({ to, src, alt, text, className = "" }) {
  return (
    <Link to={to} className={`block ${className}`}>
      <div className="group flex h-full min-h-36 w-full items-end justify-center overflow-hidden rounded-2xl bg-gray-400 p-4">
        <div className="flex-1 text-xl font-medium text-white">{text}</div>
